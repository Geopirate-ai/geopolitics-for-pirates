...
- **XII · La Cripta** (`id="cripta"`): análisis de las **cuatro altcoins de mayor capitalización** del momento (hoy: Ethereum, XRP, BNB, Solana; verifica el ranking real). 4 tarjetas (`reads`); cada una: nombre + precio aproximado (`.coin-price`), un `<p>` con **ecosistema y comportamiento**, y un bloque `.horizons` con perspectivas a **Corto / Medio / Largo plazo** (`.hz` → `.hz-t` + `<p>`), planteadas como escenarios con su riesgo, nunca como predicción. Actualiza precios y datos con búsqueda web. **Obligatorio** conservar el `<p class="disclaimer">`: la cripto es muy volátil, no es asesoría de inversión. Nunca digas "compra"; describe.
- **XIII · El Cofre** (`id="cofre"`): 12 fuentes equilibradas entre perspectivas (suele cambiar poco).
